...
public class TabeleRFSFieldToFieldAutomat {
    ...
    TabeleRFSFieldToFieldAutomat(String paramsPlikWsadowy, String paramsTrzyliterowySkrot ,  String paramsNazwaDokumentu, String paramsPlanowanaData ,String paramsKatalogDocelowy)
    {
        ...
        try {
            ...
            outputFile = paramsKatalogDocelowy + "\\Pliki_RFS_Field_To_FIeld_" + nazwaPliku +".xls";
            ...
            for (i = wartoscPoczatkowa; i <= 10000; i++)
            {
                ...
                sh1.getRow(three).createCell(2).setCellValue("Field-to-field");
                sh1.getRow(three).createCell(3).setCellValue(nazwaPliku + " -> "  + "Field to field");


                //P1
                sh1.getRow(four).createCell(1).setCellValue("P" + i);
                sh1.getRow(four).createCell(2).setCellValue("Zakładka: " + "Field-to-field.\n"
                        + "ID raportu: " + nazwaPola);
                ...
                sh1.getRow(four).createCell(3).setCellValue
                        ("Suma wygenerowana poprawnie.\nWynik zgodny z wartością w konsoli rekoncylacji.");

                sh1.getRow(four).createCell(4).setCellValue
                        ("1. Tester ma dostęp do konsoli rekoncyliacji.\n" +
                                "2. Tester ma dostęp do dokumentu RFS: \n" + nazwaPliku +
                                "\n3. Tester ma dostęp do bazy danych.");
                sh1.getRow(four).createCell(9).setCellValue(data);


                //Krok 1 [Wiersz 5]
                sh1.getRow(five).createCell(2).setCellValue("Sprawdź działanie algorytmu opisanego w kolumnie \'Algorytm rekoncyliacji pola\' w dokumencie "
                        + nazwaPliku + " dla raportu " + nazwaPola);
                sh1.getRow(five).createCell(1).setCellValue(1);
                sh1.getRow(five).createCell(3).setCellValue("Suma algorytm działa poprawnie.");
